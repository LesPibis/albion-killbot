...
const hasSupportHealingDone = participants =>
  participants.filter(participant => participant.SupportHealingDone > 0).length > 0;
...
exports.generateEventImage = async event => {
  ...
  // assists bar
  const drawAssistBar = (participants, x, y, width, height, radius, isDamageBar = true) => {
    let px = x;
    let py = y;
    ...
    ctx.strokeStyle = "#000";
    ctx.lineWidth = 3;
    const text = isDamageBar ? "Damage" : "Healing";
    ...
    const COLORS = isDamageBar ? DAMAGE_COLORS : HEALING_COLORS;
    ...
    participants.forEach((participant, i) => {
      ...
      if (barWidth > 60) {
        ...
        const text = Math.round(participant.hitPointPercent) + "%";
        const pw = ctx.measureText(text).width;
  ...
  const damageParticipants = event.Participants.filter(participant => {
  ...
  if (hasSupportHealingSection) {
    const healingParticipants = event.Participants.filter(participant => {
      if (participant.SupportHealingDone > 0) {
        participant.hitPointDone = participant.SupportHealingDone;
        return participant;
      }
    });
    drawAssistBar(healingParticipants, 35, 1300, 1530, 40, 20, false);
  }
